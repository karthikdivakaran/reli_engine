...
class CalculationsWindow(QWidget):
    ...
    def update_values(self):
        if "Pi_T" in self.component_config["values"]:
            self.component_config["values"]["Pi_T"]["value"] = self.get_pi_value()
        self.update_items()
    ...
    def calculate(self):
        ...
        if (comp not in constants.exclude_options and type not in constants.exclude_options and
            ...
            try:
                self.resultValue.setText(f" = {eval(equation_values)} × 10⁻⁹/hour")
            except:
                self.resultValue.setText(f" Error Fetching result please restart Application")
    ...
    def update_components(self):
        ...
        self.equation.setText(self.component_config.get("formula"))
        types = []
                ...
                if comp_value:
                    ...
                    for inx in range(1,5):
                        ...
                        if text_value != "" and item_key not  in text_value:
                            continue
                        self.findChild(QLabel, f"val_label_res_{inx}").setText(item_key + " :" + comp_value)
                        break
                        # "val_label_res_4"
